...
$sql1 = "SElECT * FROM dm_user WHERE uid='{$uid}'";
$re =$db -> query($sql1); 
$row = $re->fetch_assoc();
...
if($password==$new_password)
	...
	$sql = "UPDATE dm_user SET username='{$name}',password='{$hash}',email='{$mail}' WHERE uid='{$uid}'";
	$db -> query($sql); 
...
<html lang="zh-CN">
  <head>
    ...
    <title><?php echo $rows['username']; ?>-用户编辑-动漫新闻后台管理-动漫新闻</title>
...
<form action="edituser.php?uid=<?php echo $uid;?>" method = "post">
		<div>用户名： <label  class="sr-only" >用户名</label>
        <input  name = "name" class="form-control"  value="<?php echo $row["username"] ?>">
		</div>
		<div>e-mail：<label  class="sr-only">Email address</label>
        <input  name = "mail" class="form-control"  value="<?php echo $row["email"] ?>"></div>
		<div>
		<div><h2>修改密码
		</div>
		<div class="form-group <?php echo $error['has_error']?>">
		请输入要改的密码：<label  class="sr-only">新密码</label>
        <input type="password" id="inputPassword" name = "password" class="form-control" placeholder="新密码" required>
		</div>
		<div class="form-group <?php echo $error['has_error'];	$runtime_stop = microtime(true);?>">
		再输入一次：<label  class="sr-only">Email address</label>
        <input type="password" id="inputPassword" name = "new_password" class="form-control" placeholder="再输入一次" required>
		</div>
		<div>
