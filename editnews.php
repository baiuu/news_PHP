...
<html lang="zh-CN">
  <head>
    ...
    <title>动漫新闻编辑-动漫新闻后台管理-动漫新闻</title>
...
<main>
 <div class="container">
	...
	<?php
	$sql="SELECT * FROM dm_news limit $startCount,$perNumber";
	$re = $db ->query($sql);
	...
	echo "<tr><td>",$res['tittle'],"</td>";
	echo "<td>",$res['time'],"</td>";
	echo "<td><a href='edit.php?newsid={$res['newsid']}'>编辑</a><a href='del.php?newsid={$res['newsid']}'>删除</a></td></tr>"; //显示数据库的内容
}
	echo "</table>";
	echo "</div>";
	if ($page != 1) { //页数不等于1
	$lastpage = $page-1;
	echo "<a href='editnews.php?page={$lastpage}'>上一页</a>";
}
...
{
	$nextpage=$page+1;
	echo "<a href='editnews.php?page={$nextpage}'>下一页</a>";
}
}
